...
class SpinWheelConfig {
  ...
  SpinWheelConfig({
    ...
    this.confettiController,
        ...
        assert(spinResistance >= 0 && spinResistance <= 1, 'spinResistance must be between 0 and 1'),
        assert(initialSpinAngleInDegrees == null || initialSpinAngle == 0.0, 'Cannot provide both initialSpinAngle and initialSpinAngleInDegrees'),
        assert(dividerThickness >= 0, 'dividerThickness must be greater than or equal to 0'),
        assert(labels == null || labels.length == divisions, 'labels length must match divisions'),
        assert(labelTextStyles == null || labelTextStyles.length == divisions, 'labelTextStyles length must match divisions'),
        assert(labelRotationsInDegrees == null || labelRotationsInDegrees.length == divisions, 'labelRotationsInDegrees length must match divisions'),
        assert(segmentWeights == null || (segmentWeights.length == divisions && segmentWeights.every((w) => w > 0)), 'segmentWeights length must match divisions and all weights must be > 0'),
        assert(segmentColors == null || segmentColors.length == divisions, 'segmentColors length must match divisions'),
        assert(segmentGradients == null || segmentGradients.length == divisions, 'segmentGradients length must match divisions'),
